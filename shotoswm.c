...
struct shotoswm_server {
	...
	enum shotoswm_cursor_mode cursor_mode;
	struct shotoswm_view *grabbed_view;
...
static void focus_view(struct shotoswm_view *view, struct wlr_surface *surface) {
...
static bool handle_keybinding(struct shotoswm_server *server, xkb_keysym_t sym) {
	/*
...
static void server_new_keyboard(struct shotoswm_server *server,
		struct wlr_input_device *device) {
	struct shotoswm_keyboard *keyboard =
...
static void server_new_pointer(struct shotoswm_server *server,
		struct wlr_input_device *device) {
...
static bool view_at(struct shotoswm_view *view,
...
static struct shotoswm_view *desktop_view_at(
...
static void process_cursor_move(struct shotoswm_server *server, uint32_t time) {
	/* Move the grabbed view to the new position. */
	server->grabbed_view->x = server->cursor->x - server->grab_x;
	server->grabbed_view->y = server->cursor->y - server->grab_y;
}

static void process_cursor_resize(struct shotoswm_server *server, uint32_t time) {
	/*
	 ...
	 */
	struct shotoswm_view *view = server->grabbed_view;
...
static void process_cursor_motion(struct shotoswm_server *server, uint32_t time) {
	...
	struct shotoswm_view *view = desktop_view_at(server,
...
static void begin_interactive(struct shotoswm_view *view,
