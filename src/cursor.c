    ...
    uint32_t time) 
{
	...
	if (server->cursor_mode == SHOTOSWM_CURSOR_MOVE) {
		process_cursor_move(server, time);
		return;
	} else if (server->cursor_mode == SHOTOSWM_CURSOR_RESIZE) {
...
void server_cursor_button(struct wl_listener *listener, void *data) {
	...
	if (event->state == WLR_BUTTON_RELEASED) {
		...
		server->cursor_mode = SHOTOSWM_CURSOR_PASSTHROUGH;
	} else {
		/* Focus that client if the button was _pressed_ */
		shotoswm_focus_view(view, surface);
	}
}
...
struct shotoswm_view *desktop_view_at(
	...
	wl_list_for_each(view, &server->views, link) {
		if (shotoswm_view_at(view, lx, ly, surface, sx, sy)) {
			return view;
		}
	}
	return NULL;
...
void begin_interactive(struct shotoswm_view *view,
